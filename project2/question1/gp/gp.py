...
toolbox.register("expr", gp.genHalfAndHalf, pset = pset, min_ = 1, max_ = 4)#generate individual tree
...
def __stoping_criteria(pop):
    ...
    for i, child in enumerate(pop):
        #print child, child.fitness.values[0]
...
def main(population, crossrate, mutation_rate):
    ...
    #pop, log = algorithms.eaSimple(pop, toolbox, 0.5, 0.1, 300)
...
if __name__ == "__main__":
    ...
    write_file(result, "default.csv", ["default"])
    ...
    for i in range(round_):
        ...
        #result[str(child)] = generation
        result.append([generation])
    write_file(result, "large_pop.csv", ["large_pop"])

    #paste("default.csv", "large_pop.csv", "default_large.csv")
    ...
    write_file(result, "large_mut.csv", ["large_mut"])
    population = 20
    crossrate = 0.8
    mutation_rate = 0.02
    result = []
    for i in range(round_):
        ...
        #result[str(child)] = generation
        result.append([generation])
    write_file(result, "large_cro.csv", ["large_cro"])


    #paste("large_mut.csv", "default_large.csv", "new_1.csv")
