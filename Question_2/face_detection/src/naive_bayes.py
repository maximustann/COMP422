...
def comparison(test_face_dataset_file, test_non_face_dataset_file,\
    ...
    test_face_dataset_file = os.path.basename(test_face_dataset_file)
    temp_result_file = pathname + '/temp_result_' + test_face_dataset_file
    tp = distributor(temp_result_file, test_face_dataset, train_face,\
            train_non_face)
    result.append(tp)
    result_file = pathname + '/result_' + test_face_dataset_file
    paste(temp_result_file, pathname + '/' + test_face_dataset_file, result_file)


    #calculate fp
    pathname = os.path.dirname(test_non_face_dataset_file)
    test_non_face_dataset_file = os.path.basename(test_non_face_dataset_file)
    temp_result_file = pathname + '/temp_result_' + test_non_face_dataset_file
    fp = distributor(temp_result_file, test_non_face_dataset, train_face, \
            train_non_face)
    result_file = pathname + '/result_' + test_non_face_dataset_file
    paste(temp_result_file, pathname + '/' + test_non_face_dataset_file, result_file)
    result.append(fp)
    write_file(result)
...
def write_file(result):
    ...
    start_data = [0, 0]
    end_data = [1, 1]
    if not os.path.exists("./result/tp_fp.csv"):
        with open ("./result/tp_fp.csv", 'w') as f:
            writer = csv.writer(f)
        ...
        with open ("./result/tp_fp.csv", 'a') as f:
            writer = csv.writer(f)
            writer.writerow(result)
    f.close()
